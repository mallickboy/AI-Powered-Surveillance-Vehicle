...
static esp_err_t print_handler(httpd_req_t *req) {
  for (int i=0; i<3; i++){
    myFunction();
  }
  // Serial.println("Hellow Tamal");
  return httpd_resp_send_500(req);
}

static esp_err_t control_left_motors(httpd_req_t *req) {
  char *buf = NULL;
  char _control[32];
  char _value[32];
  if (parse_get(req, &buf) != ESP_OK) {
  ...
  if (httpd_query_key_value(buf, "control", _control, sizeof(_control)) != ESP_OK 
  ...
  free(buf);
  int control = atoi(_control);
  int value = atoi(_value);
  left_motors(value, control); // speed, direction

  char response[256];
  snprintf(response, sizeof(response), "Left motor control successfully applied.\nControl [ Direction ] : %d\nValue [ Speed ]: %d", control, value);
  httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}

static esp_err_t control_right_motors(httpd_req_t *req) {
  char *buf = NULL;
  char _control[32];
  char _value[32];
  if (parse_get(req, &buf) != ESP_OK) {
  ...
  int control = atoi(_control);
  int value = atoi(_value);
  right_motors(value, control); // speed, direction

  char response[256];
  snprintf(response, sizeof(response), "Right motor control successfully applied.\nControl [ Direction ] : %d\nValue [ Speed ]: %d", control, value);
  httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
static esp_err_t control_led_blink(httpd_req_t *req) {
  char *buf = NULL;
  char _control[32];
  char _value[32];
  if (parse_get(req, &buf) != ESP_OK) {
  ...
  int control = atoi(_control);
  int value = atoi(_value);
  led_blink(control, value); // speed, direction

  char response[256];
  snprintf(response, sizeof(response), "LED control successfully applied.\nControl [ no of blinks ] : %d\nValue [ interval ]: %d", control, value);
  httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
...
void startCameraServer() {
  ...
  httpd_uri_t left_motors_uri = {
    .uri = "/left_motors",
    .method = HTTP_GET,
    .handler = control_left_motors,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  httpd_uri_t right_motors_uri = {
    .uri = "/right_motors",
    .method = HTTP_GET,
    .handler = control_right_motors,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  httpd_uri_t led_blink_uri = {
    .uri = "/led_blink",
    .method = HTTP_GET,
    .handler = control_led_blink,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &print_uri);
    httpd_register_uri_handler(camera_httpd, &left_motors_uri);
    httpd_register_uri_handler(camera_httpd, &right_motors_uri);
    httpd_register_uri_handler(camera_httpd, &led_blink_uri);
  }
